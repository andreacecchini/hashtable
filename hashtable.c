...
concatenate semplici, in cui ogni nodo ha un puntatore al nodo
successivo; l'ultimo nodo della lista ha `NULL` come successore.
...
[^1]: Sommare i codici ASCII non è una buona idea perché può produrre
      collisioni in modo troppo predicibile. La funzione `encode()`
      non andrebbe quindi usata in applicazioni reali. Una soluzione
      che viene spesso raccomandata è `djb2` descritta
      [qui](http://www.cse.yorku.ca/~oz/hash.html).
...
negativo oppure superiore alla dimensione della tabella hash. Occorre
invece usare la funzione `hash_function(h,k)`, che accetta come
parametri una tabella hash `h` e un intero arbitrario `k`, e
restituisce un intero appartenente all'insieme $\{0, \ldots,
\texttt{h->size}-1\}$ usando l'operatore modulo. Il valore prodotto da
`hash_function()` viene usato come indice dell'array `h->items[]` per
decidere in quale lista di trabocco inserire la chiave data.

Il programma [hashtable-main.c](hashtable-main.c) legge una sequenza
di comandi da standard input, o da file il cui nome va passato sulla
riga di comando. I comandi manipolano una tabella hash inizialmente
vuota, e sono descritti nella Tabella 1. Il file
[hashtable.in](hashtable.in) contiene un esempio di input.
...
Comando           Significato
----------------  ------------------------------------------------------------
_n_               Inizializza l'array dei puntatori a liste di collisione
                  alla dimensione _n_ ; questa istruzione compare una
...
unsigned long encode(const char *key)
{
    ...
    s = 0;
    /* Il ciclo seguente potrebbe causare un overflow di `s`;
       ignoriamo il problema confidando che il valore della variabile
       s "riparta" dall'inizio. Non sarebbe però corretto assumere
       ciò: in caso di overflow il risultato è "indefinito" nel senso
       che la specifica del linguaggio lascia libertà alle
       implementazioni di comportarsi come vogliono. */
    for (i=0; key[i]; i++) {
        s += key[i];
    }
    return s;
...
static HashNode *hashtable_new_node(const char *key, int value, HashNode *next)
{
    ...
    /* Per duplicare la stringa `key` sarebbe possibile usare la
       ...
       ANSI C né di C99, ma_ è inclusa nelle estensioni POSIX. Di
...
int ht_insert(HashTable *h, const char *key, int value)
{
    ...
    HashNode *new_element = ht_search(h , key);


    if(new_element == NULL){
      /* Non esiste nessun elemento con la stessa chiave*/

      /* Aggiunta del nodo alla hash table*/
      new_element = hashtable_new_node(key , value, h->items[hash_function(h,encode(key))]);
      h->items[hash_function(h,encode(key))] = new_element;


      h->values_count++;

      return 1; /* Nuovo nodo inserito */
    }
    else{
      /* Esiste  un elemento con la stessa chiave*/
      new_element->value = value;
      return 0; /* Modificato nodo gia' esistente */
    }
...
HashNode *ht_search(HashTable *h, const char *key)
{
    ...
    HashNode *list = h->items[hash_function(h, encode(key))];
    /* Cerca l'elemento nella lista di nodi */
    while(list != NULL && keys_equal(list->key , key) == 0 ){
      list = list->next;
    }
    /*
      list == NULL -> nessun elemento con la chiave utilizzata
      list != NULL -> presente elemento con la chiave utilizzata
     */
    return list;
}
...
int ht_delete(HashTable *h, const char *key)
{
    ...
    HashNode *element_to_delete = NULL;
    HashNode *list = NULL;
    /* Cerco nella hash table l'elemento da eliminare */
    element_to_delete = ht_search(h,key);
    if(element_to_delete != NULL){
       /* L'elemento da eliminare e' presente */
       list =  h->items[hash_function(h, encode(key))];
       /* Ottengo l'elemento precedente a element_to_delete */
       while(list->next != element_to_delete){
           list = list->next;
       }
        /* Elimino dalla lista e dalla memoria element_to_delete */
        list->next = element_to_delete->next;
        free_node(element_to_delete);
        /* Decremento la quantita' di elementi presenti nella hash table */
        h->values_count--;
        return 1;
    }
    return 0; /* L'elemento non e' presente quindi ritorna 0 */
}
